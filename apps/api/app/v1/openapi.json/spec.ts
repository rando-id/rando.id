// Pure OpenAPI spec builder. Lives next to route.ts but doesn't depend
// on Next.js, so it's safe to import from node scripts (scripts/render-spec.ts)
// AND from the Next handler (./route.ts).
//
// What this file does:
//   - Walks the ts-rest contract via @ts-rest/open-api to emit paths/operations
//   - Lifts the named zod schemas into components.schemas via @anatine/zod-openapi
//   - Refifies operations + nested components so inline schemas become $refs
//   - Declares tags, security schemes, named error responses
//
// Drift between the contract (packages/api-client) and this output is
// impossible at the type level — every endpoint flows through the
// contract router.
...
// ─── builder (the pure entrypoint) ──────────────────────────────────

/**
 * Build the OpenAPI 3.0 document. Pure — no I/O, no Next runtime
 * dependencies. The route handler wraps the result in NextResponse;
 * scripts (e.g. `pnpm spec:lint:static`) write it to a file.
 */
export function buildSpec(): unknown {
